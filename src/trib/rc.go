...
func (self *RC) DiskConfig(i int, s PersistentStorage) *DiskServerConfig {
	ret := new(DiskServerConfig)
	ret.Addr = self.Backs[i]
	ret.Store = s
	ret.Ready = make(chan bool, 1)
...
func (self *RC) LockServerConfig(i int, s LockStorage) *LockServerConfig {
	ret := new(LockServerConfig)
	ret.Addr = self.LockServers[i]
	ret.Store = s
	ret.Ready = make(chan bool, 1)
