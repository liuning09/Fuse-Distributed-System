...
type LockStorage interface {
	TestAndSet(kv *KeyValue, succ *bool) error

	Renew(kv *KeyValue, succ *bool) error

	Release(kv *KeyValue, succ *bool) error
	
	Debug(key string, value *map[string]*Holder) error
}
